...
async function createTeam(payload) {
  ...
  const vals = {
    ...
    leader_email: clean(payload.leaderEmail, 160).toLowerCase(),
    leader_phone: clean(payload.leaderPhone, 25)
  };
  ...
  for (let attempt = 0; attempt < 10; attempt++) {
    ...
    if (response.ok) {
      ...
      return {
        teamId: created[0]?.team_id || teamId,
        createdAt: created[0]?.created_at || row.created_at
      };
    }
    ...
    /*
      If the generated Team ID happened to collide,
    ...
    */
    ...
    console.error('Supabase registration error:', errorText);
...
async function getTeams() {
  ...
  if (!response.ok) {
    ...
    throw new Error('Could not load registered teams.');
  }
...
const server = createServer(async (req, res) => {
  ...
  try {
    ...
    if (
      ...
      const result = await createTeam(b);
    ...
    if (p.startsWith('/api/admin/')) {
      ...
      if (
        ...
        const today =
          teams.filter(
            team =>
              String(team.created_at).slice(0, 10) ===
              todayString
          ).length;
